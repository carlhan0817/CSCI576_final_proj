...
| "Which parts are repeated across episodes?" | `recap` 标签 (如实现跨视频比较则扩展) |
...
│                                                         │
│  FastAPI: 静态文件服务 (MP4) + JSON API (metadata)       │
...
| 决策 | 选择 | 理由 |
|---|---|---|
| 分析与播放解耦 | 离线生成 JSON，在线只读 | 分析耗时大 (可能几分钟)，播放必须流畅；解耦后演示稳定 |
| 融合策略 | **规则引擎 + 预训练多模态嵌入 + 轻量分类器** (Plan C) | 无需标注数据；在多种视频类型上泛化；完全离线 |
| 前端形态 | FastAPI + HTML/JS (非桌面 GUI) | 跨平台演示方便，浏览器原生支持 MP4 Range 请求 |
...
#### 4.2.1 视觉子管线
**提取的特征**:
- **镜头切换点 (shot boundaries)** — 相邻帧颜色直方图差异超阈值
- **黑帧 / 纯色帧检测** — 帧的平均亮度与方差
- **运动强度** — 相邻帧光流 / 帧差绝对值
...
#### 4.2.2 音频子管线
**提取的特征**:
- **RMS 能量包络** (秒级) — 检测沉默 / 响度突变
- **频谱质心、频谱展宽、过零率** — 区分语音 / 音乐 / 噪声
...
#### 目的
基于 `metadata.json` 与原始 MP4，构建一个**可交互的 Web 播放器**，满足课程要求的三项能力：
1. 音视频严格同步播放 (浏览器原生 `<video>` 标签已保证)
2. 可视化、可点击的时间轴 (segment 以颜色区分)
3. 播放控制：播放 / 停止 / 跳到下一段 / 跳过所有非内容 / 仅播放核心内容
...
- **轻依赖**：前端不用 npm / 构建工具，一套 `.html` + `.css` + `.js` 文件即可演示。
...
**文档结束。** 本 PDD 为 V2.0;后续若技术选型有调整,在此基础上修订并提升版本号。
